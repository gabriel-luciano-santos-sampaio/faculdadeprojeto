...
form.addEventListener('submit', function(event) {
    ...
    const celularRaw = document.getElementById('celular').value.replace(/\D/g, '');
    const genero = document.querySelector('input[name="genero"]:checked')?.value;
    const exame = document.getElementById('exame').value;
    const data = document.getElementById('data').value;

    // Aqui, use celularRaw ao invés de celular
    const celular = celularRaw; // Definindo celular aqui
    ...
    fetch('processa_agendamento.php', {
        ...
        body: new URLSearchParams({
            nome: nome,
            cpf: cpfRaw,
            celular: celular, // Agora está definido corretamente
            genero: genero,
            exame: exame,
            data: data
        })
    ...
    .then(data => {
        console.log(data); // Mostra a resposta do PHP
    })
